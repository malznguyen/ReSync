...
import type { PaginatedResponse, Visit } from "@/types/api";
...
export async function GET(request: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    const path = `/analytics/visits${query ? `?${query}` : ""}`;
    const { data } = await createBackendApi(token).get<PaginatedResponse<Visit>>(
      path
    );
